...
async function fetchPexelsHeroImage(
  query: string,
  target: HeroTarget | undefined,
  config: HeroImageConfig,
): Promise<PexelsHeroImage | null> {
...
export function replaceHeroImage(html: string, hero: PexelsHeroImage): string {
...
export function detectHeroTarget(html: string): HeroTarget | null {
...
function ensureDefaultParams(url: string): string {
  try {
