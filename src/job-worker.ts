...
let pgPool: Pool;
const templateCache = new Map<string, PagePayload[]>();
const localizedTitleCache = new Map<string, string>();
...
export function triggerJobWorker(): void {
    if (!started)
        return;
...
async function fetchTemplatePagesFromWp(templateSlug: string): Promise<{ template: string; pages: PagePayload[] }> {
    const sql = `
...
async function loadTemplatesFromExport(templateSlug: string): Promise<PagePayload[]> {
...
function findAllTextNodes(html: string): string[] {
    if (!html.trim())
        return [];
    const $ = load(html, { decodeEntities: false } as unknown as CheerioOptions);
    const texts: string[] = [];
    $('*')
        .contents()
        .each((_, node) => {
...
function inferPageIntent(slug: string): string {
    ...
    const hint = TEMPLATE_PAGE_HINTS[last as keyof typeof TEMPLATE_PAGE_HINTS];
    return hint?.intent ?? '';
}
                ...
                if (newSlug && newSlug !== originalSlug) {
                    ...
                    while (ancestor && ancestor.type === 'tag') {
                        ...
                        if (idAttr && slugify(idAttr) === originalSlug) {
                            (ancestor.attribs as Record<string, string>).id = newSlug;
                            break;
                        }
